...
JonDoBrowser_GetUserDataDir(nsIFile *aExeFile, nsIFile** aFile)
{
  ...
  nsAutoCString tbDataLeafName(NS_LITERAL_CSTRING("JonDoBrowser-Data"));
  nsCOMPtr<nsIFile> appRootDir;
  nsresult rv = GetAppRootDir(aExeFile, getter_AddRefs(appRootDir));
  NS_ENSURE_SUCCESS(rv, rv);
#ifndef XP_MACOSX
  // On all platforms except Mac OS, we always operate in a "portable" mode
  // where the JonDoBrowser-Data directory is located next to the application.
  ...
  // path contains "/Applications" or (2) the JonDoBrowser-Data directory does
  ...
  if (!useOSLocation) {
    ...
    // commit to that, let's ensure that we can create the JonDoBrowser-Data
  ...
  if (useOSLocation) {
    // We are using ~/Library/Application Support/JonDoBrowser-Data. We do not
  ...
  NS_ENSURE_SUCCESS(rv, rv);
  rv = tbDataDir->AppendNative(NS_LITERAL_CSTRING("JonDoBrowser"));
  NS_ENSURE_SUCCESS(rv, rv);
#endif
